                        ...
                        <div class="col-lg-4 p-3 rounded card bg-{{ $bg }}">
                            <div class="shadow card-body border-danger">
                                ...
                                <h4 class="text-{{ $text }}">Minimum amount: <strong style="float:right;">
                                        {{ $settings->currency }}{{ $method->minimum }}</strong></h4><br>

                                <h4 class="text-{{ $text }}">Maximum amount:<strong style="float:right;">
                                        {{ $settings->currency }}{{ $method->maximum }}</strong></h4><br>

                                <h4 class="text-{{ $text }}">Charges (Fixed):<strong style="float:right;">
                                        {{ $settings->currency }}{{ $method->charges_fixed }}</strong></h4><br>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body bg-{{ $bg }}">
                                        <form style="padding:3px;" role="form" method="post"
                                            action="{{ action('App\Http\Controllers\SomeController@withdrawal') }}">
                                            <input class="form-control text-{{ $text }} bg-{{ $bg }}"
                                                placeholder="Enter amount here" type="text" name="amount"
                                                required><br />
                                            <input class="form-control text-{{ $text }} bg-{{ $bg }}"
                                                placeholder="Enter wallet here" type="text" name="wallet"
                                                required><br />
                                            <input class="form-control text-{{ $text }} bg-{{ $bg }} "
                                                value="{{ $method->name }}" type="text" disabled><br />
                                            <p class="text-{{ $text }} font-weight-bold">Provide only BEP20 Wallet as to any other wallet will result in a complete
                                                loss of funds </p>
